...
var LITERAL         = 0
  ...
  , literalLength = function() {
      ...
      if (length <= 59) {
        ...
        bytes = length - 59
        if (bytes > 1)
          throw new Error('literals spanning over 1 byte not implemented')

        length = this.input[this.inputPtr]
        length = length + 1
        this.inputPtr = this.inputPtr + 1
      }
  ...
  , oneByteCopy = function() {
      var length = ((this.input[this.inputPtr] >> 2) & 7) + 4
        , offset = ((this.input[this.inputPtr] >> 5) << 7) | this.input[this.inputPtr + 1]
